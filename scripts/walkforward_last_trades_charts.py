...
def main() -> int:
    ...
    # Snapshot strategy TOMLs (and matching env overrides) so the report directory is
    # self-contained for later replication. Mirrors walkforward_trade_recap_report.py.
    ...
    config_bundle = snapshots_to_json(
        config_snapshots,
        extra={
            "replay_env_base": dict(_REPLAY_ENV),
...
<head>
  ...
  <style>
    ...
    pre {{ background: #f4f4f5; padding: 0.75rem 1rem; border-radius: 6px; overflow: auto; font-size: 0.78rem; line-height: 1.4; }}
    pre code {{ background: transparent; padding: 0; }}
    details > summary {{ cursor: pointer; padding: 0.25rem 0; color: #2563eb; font-weight: 500; }}
    .muted {{ color: #555; font-size: 0.9rem; }}
    .neg {{ color: #b91c1c; }}
    h2#config-snapshot {{ border-top: 2px solid #ddd; padding-top: 1.25rem; margin-top: 2rem; }}
  </style>
</head>
<body>
  ...
  <p class="muted">Reproducibility: <a href="strategy_configs.json">strategy_configs.json</a> · <a href="#config-snapshot">replay config (exact TOML) ↓</a></p>
