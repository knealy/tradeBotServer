...
### Recommended: Use Maturin (Handles macOS linking automatically)

**⚠️ Important**: Use `pip3` (not `pip`) to ensure Python 3 is used!

```bash
# 1. Navigate to project (activate venv if you have one)
cd /Users/knealy/tradeBotServer
source venv/bin/activate 2>/dev/null || true  # Activate venv if it exists

# 2. Install Maturin with Python 3
pip3 install maturin
# OR: python3 -m pip install maturin

# 3. Build and Install (from rust/ directory)
cd rust
...
**If you get "unsupported hash type" errors**: You're using Python 2.7. Use `pip3` instead of `pip`.

### Alternative: Manual Build (Advanced)

```bash
# If you get linker errors with cargo build, see rust/QUICKSTART_BUILD.md
...
**Note**: If you encounter linker errors on macOS, use Maturin (recommended) or see `rust/QUICKSTART_BUILD.md` for troubleshooting.
