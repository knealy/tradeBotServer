...
- **`config/strategies/body_reversion.toml`** / **`overnight_range.toml`** — BONGO follow-through: MES **`require_bb_touch = true`** (OR with range expand); MGC **`atr_regime_quantile = 0.65`**; overnight **`range_min_pts` 20**, **`breakeven_profit_points` 8** (validate vs slip before leaning on live).
