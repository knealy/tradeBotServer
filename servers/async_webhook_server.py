...
class AsyncWebhookServer:
    ...
    def _setup_static_routes(self):
        ...
        if static_dir.exists():
            ...
            self.app.router.add_get('/admin', serve_frontend)
            self.app.router.add_get('/login', serve_frontend)
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_get('/api/auth/me', self.handle_get_current_user)
        
        # Admin endpoints
        self.app.router.add_get('/api/admin/users', self.handle_admin_list_users)
        self.app.router.add_post('/api/admin/users', self.handle_admin_create_user)
    ...
    async def handle_login(self, request: web.Request) -> web.Response:
        ...
        try:
            from infrastructure.auth_manager import get_auth_manager
            ...
            if not token:
                return web.json_response({"error": "Failed to generate token"}, status=500)
    ...
    def _require_admin(self, request: web.Request) -> Optional[Dict[str, Any]]:
        """Require admin role and return user payload."""
    ...
    async def handle_admin_list_users(self, request: web.Request) -> web.Response:
        """List all users (admin only)."""
        try:
            user_payload = self._require_admin(request)
            if not user_payload:
    ...
    async def handle_admin_create_user(self, request: web.Request) -> web.Response:
    ...
    async def handle_admin_stats(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            stats = {
                "users": {
                    "total": len(get_auth_manager().list_users()),
                    "active": len([u for u in get_auth_manager().list_users() if u.is_active]),
                },
                "trading": {
                    "selected_account": self.trading_bot.selected_account.get('name') if self.trading_bot.selected_account else None,
                    "active_strategies": len([s for s in self.trading_bot.strategy_manager.strategies.values() if s.enabled]),
                },
                "cache": {},
            }
            
            # Add Redis stats if available
            if hasattr(self.trading_bot, 'redis_cache') and self.trading_bot.redis_cache:
                stats["cache"]["redis"] = self.trading_bot.redis_cache.get_stats()
