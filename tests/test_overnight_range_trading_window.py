...
def test_in_trading_window_wrap_evening_and_early_morning():
    ...
    tz = strat.timezone
    assert strat.overnight_end == "9:29"
...
def test_replay_order_window_zero_requires_market_open_et():
    ...
    at_open = tz.localize(datetime(2026, 1, 5, 9, 30, 0))
...
def test_filter_pct_matches_shipped_overnight_range_toml():
    """Stock ``overnight_range.toml`` uses explicit *_pct matching legacy MNQ pts @ 21k."""
    ...
    assert strat.filter_gap_max_pct == pytest.approx((250.0 / ref) * 100.0)
    assert strat.filter_atr_min_pct == pytest.approx((10.0 / ref) * 100.0)
    assert strat.filter_atr_max_pct == pytest.approx((220.0 / ref) * 100.0)
